...
try {
        ...
        $counterA = 0
        do {
            if ($varJITPolicies[$counterA].PolicyType -eq "AdHocElevate") {
                $result = $varJITPolicies.Add($varJITPolicies[$counterA])
            }
            $counterA = $counterA + 1
        ...
        $counterPolicies = 0
        do {
            if ($varPolicies.Policies[$counterPolicies].Active -eq "True") {
                $result2 = $varJITActivePolicies.Add($varPolicies.Policies[$counterPolicies])
            }
        ...
        do {
            ...
            if ($resultingManualRequestsInSet.Count -eq 0){
                ...
                if ($addNewLine -eq "true") {
                    $result3 = $resultingManualRequestsInSet.Add($ManualRequestsInSet[$counterDeleteDuplicates])
                }
            }
